...
#########################################################################
def return_contact_days_sick_nodes(node_health, seed_date, G_raw,  null_comparison):
...
######################################################333
"""
def plot_results(sampler, model_type):

	fig, ax = plt.subplots(ncols=2, nrows=3)
	fig.set_size_inches(15.5, 20.5)
        ax[0,0].plot(sampler.chain[0, :, :, 0].T, color="k", lw=0.1)
        ax[0,0].set_ylabel("Walker positions for $\mu_learn$")
        ax[0,0].set_xlabel("Simulation step")

	ax[0,1].plot(sampler.chain[0, :, :, 1].T, color="k", lw=0.1)
        ax[0,1].set_ylabel("Walker positions for time-interval")
        ax[0,1].set_xlabel("Simulation step")
        
        
        samples = sampler.chain[0, :, :, 0].reshape((-1, 1))

        xplot = np.linspace(0, 0.1, 100)
        prior = [np.exp(0) for xi in xplot]
        ax[1,0].plot(xplot, prior, label="prior")
        ax[1,0].hist(samples, bins=50, histtype="step", normed=True,
                 label="posterior", color="k", linewidth=2)
        ax[1,0].axvline(0.015, label="data point", color="r")
        ax[1,0].legend(frameon=False, loc="best")
        ax[1,0].set_xlabel("$\mu$ posterior")


	samples = sampler.chain[0, :, :, 1].reshape((-1, 1))
	xplot = np.linspace(2, 15, 100)
        prior = [np.exp(0) for xi in xplot]
        ax[2,0].plot(xplot, prior, label="prior")
        ax[2,0].hist(samples, bins=50, histtype="step", normed=True,
                 label="posterior", color="k", linewidth=2)
        ax[2,0].axvline(11, label="data point", color="r")
        ax[2,0].legend(frameon=False, loc="best")
        ax[2,0].set_xlabel("time_step posterior")

	mean_lnlikes = np.mean(np.mean(sampler.lnlikelihood, axis=1), axis=1)
        betas = sampler.betas
        ax[2,1].semilogx(betas, mean_lnlikes, "-o")
        ax[2,1].set_xlabel(r"$\beta$")
        ax[2,1].set_ylabel("Mean likelihood")

        
	
	plt.tight_layout()
        plt.savefig(model_type+"_summary.png")
"""
########################################################################
...
######################################################333
def plot_results(sampler, nburn, ndim):
 	...
 	plt.tight_layout()
        plt.savefig("model_summary_var_selection_v2_short.png")
